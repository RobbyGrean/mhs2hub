...
function generateReport(mode) {
    ...
    // --- 1. จัดการเรื่องซ่อน Card หน้าหลัก (ถ้าพี่อยากให้ซ่อนตอนกดพิมพ์) ---
    if (previewCard) {
    ...
    if (mode === 'success') {
        ...
        const successData = baseFiltered.filter(item => item.status === 'ออกแล้ว')
                                        .sort((a, b) => a.name.localeCompare(b.name, 'th'));
        totalCount = successData.length;
        if (totalCount === 0) {
            if (previewCard) { previewCard.classList.remove('hidden'); previewCard.style.display = 'block'; }
            return alert("ไม่มีรายการที่ออกแล้วครับ");
        }

        contentHTML = `
            <h3 style="margin: 15px 0 10px 0; color: #059669;">✅ รายการที่ดำเนินการแล้ว (${totalCount} เขต)</h3>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <thead><tr style="background: #f1f5f9;"><th style="border: 1px solid #000; padding: 10px; width: 60px;">ลำดับ</th><th style="border: 1px solid #000; padding: 10px; text-align: left;">ชื่อเขตพื้นที่</th><th style="border: 1px solid #000; padding: 10px; width: 150px;">สถานะ</th></tr></thead>
        ...
        if (totalCount === 0) {
            if (previewCard) { previewCard.classList.remove('hidden'); previewCard.style.display = 'block'; }
            return alert("ไม่มีรายการค้างดำเนินการครับ");
        }

        contentHTML = `
            ${overDue.length > 0 ? `<h3 style="margin: 15px 0 10px 0; color: #e11d48;">⚠️ รายการค้างจ่าย (${overDue.length} เขต)</h3>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <thead><tr style="background: #f1f5f9;"><th style="border: 1px solid #000; padding: 10px; width: 60px;">ลำดับ</th><th style="border: 1px solid #000; padding: 10px; text-align: left;">ชื่อเขตพื้นที่</th><th style="border: 1px solid #000; padding: 10px; width: 150px;">สถานะ</th></tr></thead>
                <tbody>${renderRows(overDue)}</tbody>
            </table>` : ''}
            ${noData.length > 0 ? `<h3 style="margin: 25px 0 10px 0; color: #475569;">⚪ รายการยังไม่มีข้อมูล (${noData.length} เขต)</h3>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                <thead><tr style="background: #f1f5f9;"><th style="border: 1px solid #000; padding: 10px; width: 60px;">ลำดับ</th><th style="border: 1px solid #000; padding: 10px; text-align: left;">ชื่อเขตพื้นที่</th><th style="border: 1px solid #000; padding: 10px; width: 150px;">สถานะ</th></tr></thead>
    ...
    printWindow.document.write(`
        <html>
            <head>
                ...
                <style>
                    body { font-family: 'Kanit', sans-serif; padding: 30px; color: black; background: white; }
                    .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 15px; margin-bottom: 25px; }
                </style>
            </head>
            <body>
                <div class="header">
                    <h1 style="margin: 0; font-size: 24px; color: ${colorTheme};">${reportTitle}</h1>
                    <p style="font-size: 16px; margin: 8px 0;">เดือน : ${targetMonth}</p>
                    <p style="font-size: 12px; color: #666;">พิมพ์เมื่อ: ${new Date().toLocaleString('th-TH')} น.</p>
                </div>
                ${contentHTML}
                <div style="text-align: right; font-weight: bold; font-size: 16px; margin-top: 20px;">รวมทั้งสิ้น: ${totalCount} เขต</div>
            </body>
        </html>
    `);
    printWindow.document.close();
    
    // --- 3. สั่งพิมพ์ และคืนค่า Card หน้าหลัก ---
    setTimeout(() => {
        printWindow.focus();
        printWindow.print();
        printWindow.close();
